...
Generic OpenWrt 已经不再只是只读诊断：真实 iStoreOS x86_64 已完成 detect / preflight / stage / 临时 runtime / UU 云连接 / UU账号绑定 / 手机移动数据 Remote WOL 功能终验、真实 `/overlay` 持久安装，以及真实 reboot 后 procd 自动恢复。**真实 rollback 尚未验收**，因此继续保持 Experimental，而不是直接标成完整 Verified。
