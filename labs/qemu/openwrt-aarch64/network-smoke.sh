...
fail() {
    printf 'qemu-arm64-network-lab: fail\nreason: %s\n' "$1" >&2
    exit "${2:-1}"
}
...
printf '%s\n' "Starting QEMU ARM64 network/API smoke-test"

expect "$SCRIPT_DIR/network-smoke.exp" "$IMAGE_PATH"

printf '%s\n' "QEMU_ARM64_NETWORK_SMOKE_PASS"
