...
IMAGE_PATH="$WORK_DIR/openwrt-arm64-persistent.img"

fail() {
    printf 'qemu-arm64-persistent-lab: fail\nreason: %s\n' "$1" >&2
    exit "${2:-1}"
}
...
if [ ! -f "$IMAGE_GZ_PATH" ]; then
    ...
    if command -v curl >/dev/null 2>&1; then
        curl -fL --retry 3 --connect-timeout 15 \
            "$OPENWRT_BASE_URL/$IMAGE_GZ_NAME" -o "$IMAGE_GZ_PATH"
...
# Always start the lifecycle test from a fresh copy of the pinned image. The
# same raw disk is then reused across all three guest boots inside one run.
gzip -dc "$IMAGE_GZ_PATH" > "$IMAGE_PATH" || fail "unable to decompress persistent OpenWrt image" 4

printf '%s\n' "OpenWrt persistent image SHA256: pass"
printf '%s\n' "Starting QEMU ARM64 persistent install/reboot/rollback lifecycle"

expect "$SCRIPT_DIR/persistent-smoke.exp" "$IMAGE_PATH" "$EFI_PATH" "$ROOT_DIR"

printf '%s\n' "QEMU_ARM64_PERSISTENT_LIFECYCLE_PASS"
