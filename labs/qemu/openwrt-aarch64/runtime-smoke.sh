...
fail() {
    printf 'qemu-arm64-runtime-lab: fail\nreason: %s\n' "$1" >&2
    exit "${2:-1}"
}
...
printf '%s\n' "Starting QEMU ARM64 NetEase runtime smoke-test"

expect "$SCRIPT_DIR/runtime-smoke.exp" "$IMAGE_PATH" "$ROOT_DIR"

printf '%s\n' "QEMU_ARM64_RUNTIME_SMOKE_PASS"
